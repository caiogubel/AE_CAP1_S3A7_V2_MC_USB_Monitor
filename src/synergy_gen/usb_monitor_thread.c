...
TX_THREAD usb_monitor_thread;
void usb_monitor_thread_create(void);
static void usb_monitor_thread_func(ULONG thread_input);
/** Alignment requires using pragma for IAR. GCC is done through attribute. */
#if defined(__ICCARM__)
#pragma data_alignment = BSP_STACK_ALIGNMENT
#endif
static uint8_t usb_monitor_thread_stack[1024] BSP_PLACE_IN_SECTION(".stack.usb_monitor_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
{ .count_source = AGT_CLOCK_LOCO, .agto_output_enabled = false, .agtio_output_enabled = false, .output_inverted = false,
  .agtoa_output_enable = false, .agtob_output_enable = false, };
static const timer_cfg_t g_timer0_cfg =
{ .mode = TIMER_MODE_PERIODIC, .period = 500, .unit = TIMER_UNIT_PERIOD_MSEC, .channel = 0, .autostart = true,
  .p_callback = agt_cb, .p_context = &g_timer0, .p_extend = &g_timer0_extend, .irq_ipl = (3), };
/* Instance structure to use this module. */
const timer_instance_t g_timer0 =
{ .p_ctrl = &g_timer0_ctrl, .p_cfg = &g_timer0_cfg, .p_api = &g_timer_on_agt };
TX_EVENT_FLAGS_GROUP g_s3a7_st_flags;
...
void usb_monitor_thread_create(void)
{
    ...
    UINT err_g_s3a7_st_flags;
    err_g_s3a7_st_flags = tx_event_flags_create (&g_s3a7_st_flags, (CHAR *) "New Event Flags");
    if (TX_SUCCESS != err_g_s3a7_st_flags)
    {
        tx_startup_err_callback (&g_s3a7_st_flags, 0);
    }

    UINT err;
    err = tx_thread_create (&usb_monitor_thread, (CHAR *) "USB_monitor_ Thread", usb_monitor_thread_func, (ULONG) NULL,
                            &usb_monitor_thread_stack, 1024, 1, 1, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
        tx_startup_err_callback (&usb_monitor_thread, 0);
    }
}
