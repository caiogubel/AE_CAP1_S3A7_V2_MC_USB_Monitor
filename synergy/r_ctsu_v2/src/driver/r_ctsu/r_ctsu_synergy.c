...
ctsu_err_t R_CTSU_Scan(ctsu_ctrl_t * const p_ctrl)
{
    ...
    if (((ctsu_instance_ctrl_t*)p_scan_ctrl != p_ctsu_ctrl) || (true == p_ctsu_ctrl->reload))
    {
        ...
        HW_CTSU_CTSUCR1Set(p_ctsu_ctrl->p_ctsu_settings->ctsucr1.byte);     //Set control reg1
        HW_CTSU_CTSUSDPRSSet(p_ctsu_ctrl->p_ctsu_settings->ctsusdprs.byte); //Synch noise reduction reg
        HW_CTSU_CTSUDCLKCSet(p_ctsu_ctrl->p_ctsu_settings->ctsudclkc.byte); //CTSU High-Pass Noise Reduction control reg
        HW_CTSU_CTSUSSTSet(p_ctsu_ctrl->p_ctsu_settings->ctsusst.byte);     //CTSU Sensor Stabilization Wait Control Register
        HW_CTSU_CTSUCHAC0Set(p_ctsu_ctrl->p_ctsu_settings->ctsuchac0.byte); //CTSU Channel Enable Control Register 0-4
        ...
        HW_CTSU_CTSUCHTRC0Set(p_ctsu_ctrl->p_ctsu_settings->ctsuchtrc0.byte);   //CTSU Channel Transmit/Receive Control Register 0-4 (tx = 1)
        ...
        HW_CTSU_CTSUCR0Set((p_ctsu_ctrl->p_ctsu_settings->ctsucr0.byte & (uint8_t)0xFE)); //Set control reg 0 except start measurement
...
ctsu_err_t R_CTSU_Control(ctsu_ctrl_t *const p_ctrl, ctsu_control_arg_t* p_arg)
{
    ...
    {
        /* handle situation for CTSU_CMD_SET_TWO_ARG_LAST > cmd */
        ctsu_control_arg_ext_t* p_context = (ctsu_control_arg_ext_t*)p_arg->p_context;
#if (CTSU_CFG_PARAM_CHECKING_ENABLE > 0)
        ASSERT(p_ctrl_blk->num_sensors > p_context->offset);
#endif
...
ctsu_err_t R_CTSU_Calibrate(ctsu_ctrl_t * const p_ctrl)
{
    ...
    while(num_sensors_tuned < num_sensors)
    {
        ...
        for(sensor_itr = 0; (sensor_itr < num_sensors) ; sensor_itr++)
        {
            ...
            ctsuso = p_ctsu_ctrl->p_sensor_settings[sensor_itr].ctsuso0 & 0x3FF; //Read the CTSU Sensor Offset Adjustment
