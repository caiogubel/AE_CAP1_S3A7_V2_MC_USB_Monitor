...
static ssp_err_t agt_source_freq_get(agt_count_source_t count_source, uint32_t * p_clk_freq_hz)
{
    ...
    else if (AGT_CLOCK_AGT0_UNDERFLOW == count_source)
    {
        ...
        agt_source_freq_get(agt0_count_source, p_clk_freq_hz);
        *p_clk_freq_hz /= g_agt_channel0_period;
    }
    else
    {
                                      ...
                                      uint64_t    *       p_period_counts)
{
    ...
    switch (unit)
    {
        ...
        case TIMER_UNIT_FREQUENCY_KHZ:
            /** counts = src_clock_freq (Hz) / (1000 kHz/Hz * desired_freq (kHz)) */
            *p_period_counts = (clk_freq_hz * 1ULL) / (AGT_KHZ_PER_HZ * (uint64_t) period);
            break;
        case TIMER_UNIT_FREQUENCY_HZ:
            /** counts = src_clock_freq (Hz) / (desired_freq (Hz) */
            *p_period_counts = (clk_freq_hz * 1ULL) / period;
            break;
        case TIMER_UNIT_PERIOD_NSEC:
            /** counts = desired_freq (ns) * src_clock_freq (Hz) / (1000000000 ns/s) */
            *p_period_counts = (period * (clk_freq_hz * 1ULL)) / AGT_NSEC_PER_SEC;
            break;
        case TIMER_UNIT_PERIOD_USEC:
            /** counts = desired_freq (us) * src_clock_freq (Hz) / (1000000 us/s) */
            *p_period_counts = (period * (clk_freq_hz * 1ULL)) / AGT_USEC_PER_SEC;
            break;
        case TIMER_UNIT_PERIOD_MSEC:
            /** counts = desired_freq (ms) * src_clock_freq (Hz) / (1000 ms/s) */
            *p_period_counts = (period * (clk_freq_hz * 1ULL)) / AGT_MSEC_PER_SEC;
            break;
        case TIMER_UNIT_PERIOD_SEC:
            /** counts = desired_freq (s) * src_clock_freq (Hz) */
            *p_period_counts = period * (clk_freq_hz * 1ULL);
            break;
        default:
